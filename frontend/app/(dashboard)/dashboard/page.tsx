...
export default function Dashboard() {
  ...
  const dadosMensais = meses.map(m => {
    const rec = transacoes.filter(t => { const d = new Date(t.data); return t.tipo === 'receita' && d.getMonth() === m.mes && d.getFullYear() === m.ano }).reduce((acc, t) => acc + Number(t.valor), 0)
    const desp = transacoes.filter(t => { const d = new Date(t.data); return t.tipo === 'despesa' && d.getMonth() === m.mes && d.getFullYear() === m.ano }).reduce((acc, t) => acc + Number(t.valor), 0)
    return { ...m, rec, desp }
  })
  ...
  // Comparativo calculado após transacoes carregadas
      ...
      <div className="max-w-5xl mx-auto p-4 md:p-6 lg:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div className="bg-white dark:bg-gray-900 rounded-xl border dark:border-gray-800 p-5">
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Saldo atual</p>
            <p className={`text-3xl font-bold ${saldo >= 0 ? 'text-green-600' : 'text-red-500'}`}>
              R$ {saldo.toFixed(2)}
            </p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">{transacoes.length} transacoes no total</p>
          </div>

          <div className="bg-white dark:bg-gray-900 rounded-xl border dark:border-gray-800 p-5">
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Total receitas</p>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border dark:border-gray-800 p-5">
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Total despesas</p>
        ...
        {/* Grafico + Saude financeira */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="col-span-1 md:col-span-2 bg-white dark:bg-gray-900 rounded-xl border dark:border-gray-800 p-5">
            <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-4">Receitas x Despesas (6 meses)</h2>
            <div className="flex items-end gap-3 h-36">
              {dadosMensais.map((m, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full flex gap-1 items-end" style={{ height: '100px' }}>
                    <div className="flex-1 bg-green-100 dark:bg-green-900 rounded-t" style={{ height: `${(m.rec / maxMensal) * 100}%`, minHeight: m.rec > 0 ? '4px' : '0' }} />
                    <div className="flex-1 bg-red-100 dark:bg-red-900 rounded-t" style={{ height: `${(m.desp / maxMensal) * 100}%`, minHeight: m.desp > 0 ? '4px' : '0' }} />
                  </div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border dark:border-gray-800 p-5">
            <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-4">Saude financeira</h2>
            {totalReceitas === 0 ? (
              ...
              <>
                <div className="mb-4">
                  ...
                  <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${(totalDespesas / totalReceitas) > 0.8 ? 'bg-red-500' : (totalDespesas / totalReceitas) > 0.6 ? 'bg-amber-400' : 'bg-green-500'}`}
                      style={{ width: `${Math.min(100, (totalDespesas / totalReceitas) * 100)}%` }}
                    />
                  </div>
                </div>
                <p className={`text-sm font-medium ${(totalDespesas / totalReceitas) > 0.8 ? 'text-red-500' : (totalDespesas / totalReceitas) > 0.6 ? 'text-amber-500' : 'text-green-600'}`}>
        ...
        {/* Ultimas transacoes + Comparativo + Categorias */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">

          {/* Ultimas transacoes */}
          <div className="col-span-1 md:col-span-2 bg-white dark:bg-gray-900 rounded-xl border dark:border-gray-800 overflow-hidden">
            <div className="flex justify-between items-center px-5 py-4 border-b dark:border-gray-800">
              <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200">Ultimas transacoes</h2>
            ...
            {ultimasTransacoes.length === 0 ? (
              ...
              ultimasTransacoes.map(t => (
                <div key={t.id} className="flex items-center justify-between px-5 py-3 border-b dark:border-gray-800 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800">
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full flex-shrink-0 ${t.tipo === 'receita' ? 'bg-green-500' : 'bg-red-400'}`} />
                    <div>
                      <p className="text-sm font-medium text-gray-800 dark:text-gray-200">{t.categoria}</p>
          ...
          {/* Coluna direita: comparativo + categorias */}
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl border dark:border-gray-800 p-5">
              <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-4">Gastos por categoria</h2>
                  ...
                  {categoriasSorted.slice(0, 5).map(([cat, val]) => (
                    <div key={cat}>
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="text-xs font-medium text-gray-700 dark:text-gray-300">R$ {val.toFixed(2)}</span>
                      </div>
